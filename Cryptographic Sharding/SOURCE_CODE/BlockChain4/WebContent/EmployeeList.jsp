...
<!-- //banner -->
	...
	<div class="services" id="services">
		<div class="container">
			<h2 class="w3ls_head"><span>User </span>List</h2>
		</div></div>
		<div class="hei">
		<table id="customers">
       <thead>  
		...
		<tbody>
				<c:forEach items="${list}" var="e">
					<tr>
						...
						<td>
						<c:choose>
						  <c:when test="${e.loginStatus==1}">
						    <h4 STYLE="color:red;font-weight: bold;">Active</h4>
						  </c:when>
						  <c:otherwise>
						    <h4 STYLE="color:red;font-weight: bold;">Blocked</h4>
						  </c:otherwise>
						</c:choose>
						</td>
						<td> 
							<a href = "${pageContext.request.contextPath}/AdminControl?act=APPROVE&id=${e.uid}">APPROVE</a> 
							| 
							<a href = "${pageContext.request.contextPath}/AdminControl?act=BLOCK&id=${e.uid}">BLOCK</a> 
						</td>
						<td>
						<c:choose>
						  <c:when test="${e.role eq 'OWNER'}">
						  </c:when>
						  <c:otherwise>
						    <a href = "${pageContext.request.contextPath}/AdminControl?act=P&id=${e.uid}&pid=${e.pan}&name=${e.name}">GENERATE PAYSLIP</a>
						  </c:otherwise>
						</c:choose>
						</td>
