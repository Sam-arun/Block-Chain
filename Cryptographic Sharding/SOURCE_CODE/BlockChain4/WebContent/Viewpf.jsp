...
<!-- //banner -->
	...
	<div class="services" id="services">
		<div class="container">
			<h2 class="w3ls_head"><span>PF Claim</span>Requests</h2>
		</div></div>
		<div class="hei">
		<table id="customers">
       <thead>  
		...
		{
			String sta = "Applied";
			String sql = "SELECT * FROM pfreq WHERE status='"+sta+"'";			
		...
		<tbody>
					<tr>
						...
						<td><%=r.getString("UAN")%></td>
						<td><%=r.getString("Aadhar")%></td>
						<td><%=r.getString("Account")%></td>
						<td><%=r.getString("Bank")%></td>
						<td><%=r.getString("reason")%></td>
						<td><%=r.getString("status")%></td>
						<td><a href="pf_email.jsp?pan=<%=r.getString("pan")%>&id=<%=r.getString("uid")%>&pfa=<%=r.getString("pfamount")%>&acc=<%=r.getString("Account")%>">Send Status</a></td>
					</tr>
